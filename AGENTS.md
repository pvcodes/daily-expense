...
- `npm run build && npm run start` — production build with service worker + manifest
- `npm run lint` — ESLint (no errors, warnings ok)

## Structure

```
app/
  layout.tsx        # root layout: PWA meta tags, ExpenseProvider, BottomNav
  page.tsx          # dashboard (summary cards, donut, monthly bars, top categories, recent)
  transactions/     # list with category filter + note search
  categories/       # categories -> notes (tap to expand)
  settings/         # import/export CSV + JSON, clear all
...
Importer also accepts legacy 17-column format (account/amount/currency/title/...). Dates accepted: `yyyy-MM-dd`, `yyyy-MM-dd HH:mm[:ss]`, `mm/dd/yyyy HH:mm`. Quoted `"-12,34"` = -12.34. Currency is hardcoded INR for display.
